...
public:
	...
	const char *what() const noexcept override { return message_.what(); }
...
int main(int argc, char *argv[]) {
	...
	if (argc != 6 && argc != 2 && argc != 5 && argc != 3 && argc != 7 && argc != 8 && argc != 9 && argc != 10 &&
		...
		std::cout << std::endl
		          ...
		          << "Usage: cfm-predict <input_smiles_or_inchi> <prob_thresh_for_prune> <param_filename> "
		             "<config_filename> <include_annotations> <output_filename> <apply_post_processing>"
		          << std::endl
		          << std::endl
		          << std::endl;
		std::cout << std::endl
		          << "input_smiles_or_inchi_or_file:" << std::endl
		          << "The smiles or inchi string of the structure whose spectra you want to predict, or a .txt file "
		             "containing a list of <id smiles> pairs, one per line."
		          << std::endl;
		std::cout << std::endl
		          << "prob_thresh_for_prune (opt):" << std::endl
		          << "The probability below which to prune unlikely fragmentations (default 0.001)" << std::endl;
		std::cout << std::endl
		          << "param_filename (opt):" << std::endl
		          << "The filename where the parameters of a trained cfm model can be found (if not given, assumes "
		             "param_output.log in current directory)"
		          << std::endl;
		std::cout << std::endl
		          << "config_filename (opt):" << std::endl
		          << "The filename where the configuration parameters of the cfm model can be found (if not given, "
		             "assumes param_config.txt in current directory)"
		          << std::endl;
		std::cout << std::endl
		          << "include_annotations (opt):" << std::endl
		          << "Whether to include fragment information in the output spectra (0 = NO (default), 1 = YES ). "
		             "Note: ignored for msp/mgf output."
		          << std::endl;
		std::cout
		    << std::endl
		    << "output_filename_or_dir (opt):" << std::endl
		    << "The filename of the output spectra file to write to (if not given or given stdout, prints to stdout), "
		       "OR directory if multiple smiles inputs are given (else current directory) OR msp or mgf file."
		    << std::endl;
		std::cout << std::endl
		          << "postprocessing method (opt):" << std::endl
		          ...
		          << std::endl;
		std::cout << std::endl
		          << "suppress_exception (opt):" << std::endl
		          << "Suppress exceptions so that the program returns normally even when it fails to produce a result "
		             "(0 = OFF , 1 = ON (default))."
		          << std::endl;
		std::cout << std::endl
		          << "postprocessing_energy (opt):" << std::endl
		          << "postprocessing energy out of 80% (default 80%)" << std::endl;
		std::cout << std::endl
		          << "min_peak_intensity [0,100.0] (opt):" << std::endl
		          << "min amount of peak relative intensity, enter -1 to use default" << std::endl;
		std::cout << std::endl
		          << "override min peaks constraint (opt):" << std::endl
		          << "min amount of peak will include in the spectra, enter -1 to use default" << std::endl;
		std::cout << std::endl
		          << "override max peaks constraint(opt):" << std::endl
		          << "max amount of peak will include in the spectra, enter -1 to use default" << std::endl;
		std::cout << std::endl
		          << "prediction id (opt):" << std::endl
		          << "id for predicted spectra, only used in single input mode" << std::endl;
		exit(1);
	}
	...
	if (argc >= 3) {
		try {
			prob_thresh_for_prune = boost::lexical_cast<float>(argv[2]);
	...
	if (argc >= 6) {
		try {
			do_annotate = boost::lexical_cast<bool>(argv[5]);
		} catch (boost::bad_lexical_cast &e) {
	...
	if (argc >= 7) {
		...
		if (output_filename != "stdout") to_stdout = false;
	}
	if (argc >= 8) {
		try {
			postprocessing_method = boost::lexical_cast<int>(argv[7]);
		} catch (boost::bad_lexical_cast &e) {
	...
	if (argc == 9) {
		try {
			suppress_exceptions = boost::lexical_cast<bool>(argv[8]);
		} catch (boost::bad_lexical_cast &e) {
	...
	if (argc == 10) {
		try {
			postprocessing_energy = boost::lexical_cast<double>(argv[9]);
		} catch (boost::bad_lexical_cast &e) {
	...
	if (argc == 11) {
		try {
			...
			if (input_min_peak_intensity >= 0) min_peak_intensity = input_min_peak_intensity;
		} catch (boost::bad_lexical_cast &e) {
	...
	if (argc == 12) {
		try {
			...
			if (input_min_peaks >= 0) min_peaks = input_min_peaks;
		} catch (boost::bad_lexical_cast &e) {
	...
	if (argc == 13) {
		try {
			...
			if (input_max_peaks >= 0) max_peaks = input_max_peaks;
		} catch (boost::bad_lexical_cast &e) {
	...
	if (argc == 14) { single_prediction_id = argv[13]; }
	...
	if (!boost::filesystem::exists(config_filename)) {
		...
		if (!suppress_exceptions) throw FileException("Could not find file: " + param_filename);
		exit(1);
	}
	...
	if (!to_stdout && output_filename.substr(output_filename.size() - 4) == ".msp") {
		...
		if (!of.is_open()) {
			...
			if (!suppress_exceptions) throw FileException("Could not open output msp file " + output_filename);
		}
		buf = of.rdbuf();
		...
		if (!of.is_open()) {
			...
			if (!suppress_exceptions) throw FileException("Could not open output mgf file " + output_filename);
		}
		buf = of.rdbuf();
		out = new std::ostream(buf);
	} else if (!to_stdout && (output_filename.substr(output_filename.size() - 4) == ".txt" ||
		...
		if (!of.is_open()) {
			...
			if (!suppress_exceptions) throw FileException("Could not open output txt/log file " + output_filename);
		}
		buf = of.rdbuf();
		out = new std::ostream(buf);
	}
	...
	if (input_smiles_or_inchi.size() > 4 && input_smiles_or_inchi.substr(input_smiles_or_inchi.size() - 4) == ".txt") {

		try {
			parseInputFile(data, input_smiles_or_inchi, &cfg);
		} catch (FileException &e) {
			if (!suppress_exceptions) throw FileException("Could not parse input file " + input_smiles_or_inchi);
			exit(1);
		}
		batch_run = true;
	...
	for (size_t mol_idx = 0; mol_idx < data.size(); ++mol_idx) {
		...
		try {
			...
			std::cerr << "Could not sanitize input: " << mol_data.getId() << " " << mol_data.getSmilesOrInchi()
			          << std::endl;
			...
			std::cerr << "Could not parse input: " << mol_data.getId() << " " << mol_data.getSmilesOrInchi()
			          << std::endl;
			...
			std::cerr << "Could not compute fragmentation graph for input: " << mol_data.getId() << " "
			          << mol_data.getSmilesOrInchi() << std::endl;
			if (!batch_run && !suppress_exceptions)
				throw SpectrumPredictionException("Could not compute fragmentation graph for input: " +
				                                  mol_data.getSmilesOrInchi());
			continue;
		}
			...
			std::cerr << "Timeout computing fragmentation graph for input: " << mol_data.getId() << " "
			          << mol_data.getSmilesOrInchi() << std::endl;
		} catch (std::runtime_error &e) {
			// whatever else can go wrong
			std::cerr << e.what() << std::endl;
			if (!batch_run && !suppress_exceptions) throw std::runtime_error(e.what());
			continue;
		}
		...
		if (output_mode == NO_OUTPUT_MODE || output_mode == SINGLE_TXT_OUTPUT_MODE) {
			if (output_mode == SINGLE_TXT_OUTPUT_MODE && mol_idx > 0) *out << std::endl << std::endl;
		...
		if (!to_stdout)
			std::cout << "(" << mol_idx + 1 << "/" << data.size() << ") Predicted Spectra for " << mol_data.getId()
			          << " " << mol_data.getSmilesOrInchi() << std::endl;
	}
