...
int FragmentTreeNode::addChild(int e_f0, int e_to_allocate, std::vector<int> &output_bmax, Break &brk,
	...
	for (unsigned int i = 0; i < rwmol.getNumAtoms(); i++) {
		...
		// std::cout << "[DEBUG][ATOM][ID " << atom->getIdx() << "][" << atom->getSymbol() << "][Charge "
		// << atom->getFormalCharge() << "][OrigValence " << orig_val << "]" << std::endl;
		...
		// std::cout << "[DEBUG][ATOM][ID " << atom->getIdx() << "][" << atom->getSymbol() << "][numH " << numH << "]"
		// << std::endl;
	}
			...
			if (!brk.isCycliaztion()) {
				...
				if (distance > 3 && distance < 6 && (num_h_ring_root > 1) && (num_h_current_root > 1)) {
					...
					try {
						...
						std::cout << "Could not sanitize skuuuuuu" << std::endl;
						throw e;
					}
...
FragmentTreeNode::findAlreadyChargedOrSplitCharge(boost::tuple<int, int, int> &out_pidx_nidx_ridx, RDKit::RWMol &rwmol,
	...
	// "[other_side]"
	// << frag_total_qs[1 - charge_side] << std::endl;
	...
	if (!is_negative && frag_total_qs[charge_side] == 1 && frag_total_qs[1 - charge_side] == 0) {
		if (radical_side < 0 && rad_idx == -1)
			...
			for (int i = 0; i < rwmol.getNumAtoms(); i++) {
				...
				if (atom->getTotalNumHs() > 0) {
					std::cout << "[DEBUG][output before crash ? ]" << RDKit::MolToSmiles(rwmol) << std::endl;
					alterNumHs(atom, -1);
...
void FragmentTreeNode::assignChargeAndRadical(RDKit::RWMol &rwmol, int charge_idx, int radical_idx, bool is_negative) {
	...
	// std::cout << "[DEBUG][before assignChargeAndRadical]" << RDKit::MolToSmiles(rwmol) << std::endl;
	// std::cout << "[DEBUG][before assignChargeAndRadical][charge_idx]" << charge_idx << "[radical_idx]" << radical_idx
	// << std::endl;
	alterNumHs(atom, 0);
	if (is_negative) {
		atom->setFormalCharge(-1);
		...
		if (radical_idx < 0) {
			...
			rad_atom->calcExplicitValence(false);
		}
	}
