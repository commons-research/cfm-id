...
void FragmentTreeNode::generateChildrenOfBreak(Break &brk) {
	...
	for (auto &rearrangement_config : rearrangement_configs) {
		...
		if (brk.getBondIdx() != -1 && !brk.isRingBreak()) {
			...
			for (unsigned int i = 0; i < N; i++) f0_output_bmax[i] += f1_output_bmax[i];
...
int FragmentTreeNode::addChild(int e_f0, int e_to_allocate, std::vector<int> &output_bmax, Break &brk,
                               int charge_frag) {
	...
	for (int i = 0; i < numbonds; i++) {
		...
		//           bond->getEndAtomIdx() << " [remaining_e] " << remaining_e[fragidx] << " output_bmax[i] " <<
		//           output_bmax[i] << std::endl;
		int num_to_add = 0;
	...
	if (remaining_e[1 - charge_frag] > 0) return 0;

	// Remove the broken bonds
	auto itr = broken_specs.begin();
	for (; itr != broken_specs.end(); ++itr) rwmol.removeBond(itr->first, itr->second);
	...
	for (unsigned int i = 0; i < rwmol.getNumAtoms(); i++) {
		...
		if (ionic_frag_q != 0 && fragidx != charge_frag) nl_ionic_q += ionic_frag_q;
		...
		if (numH < 0) {
			...
			if (fragidx != charge_frag) split_charge_fragidx = fragidx;
		}
	...
	if ((split_charge_fragidx == (1 - charge_frag) && nl_ionic_q == 0) || nl_ionic_q > 0) {
		...
		for (unsigned int i = 0; i < rwmol.getNumAtoms(); i++) {
		std:
			std::unique_ptr<RDKit::Atom> atom = std::unique_ptr<RDKit::Atom>(rwmol.getAtomWithIdx(i));
			atom->getProp("FragIdx", fragidx);
	...
	for (int radical_frag = min_radical_frag; radical_frag <= max_radical_frag; radical_frag++) {
		...
		// Check for an already charge molecule (e.g. due to N lone pair bonding), or a molecule with split charge (in
		// which case, assign charge there)
		boost::tuple<int, int, int> pidx_nidx_ridx(-1, -1, -1);
		std::pair<int, int> qidx_ridx(-1, -1);
		boost::tuple<bool, bool, bool> qfound_oktogo_switchq =
		    findAlreadyChargedOrSplitCharge(pidx_nidx_ridx, rwmol, charge_frag, radical_frag, is_negative);
		...
		if (brk.isRingBreak()) {
			...
			for (const auto &mol : mols)
				...
				if (nullptr != getLabeledAtom(mol, "Root")) child_ion = mol;
			...
			if (!brk.isCycliaztion()) {
				...
				for (auto ai = rw_child_ion.beginAtoms(); ai != rw_child_ion.endAtoms(); ++ai) {
					...
					int is_current = 0;
					(*ai)->getProp("Root", is_current);
					if (is_current) { current_root_ai = ai; }
				}
				...
				if (distance > 3 && distance < 6 && (num_h_ring_root > 1) && (num_h_current_root > 1)) {
					...
					labelExtraBreakPropertiesInIon(child_ion, ion, brk);
					auto child_node = FragmentTreeNode(cyclizated_child_ion, child_nl, allocated_e[charge_frag],
			...
			for (int mol_idx = 0; mol_idx < mols.size(); mol_idx++) {
				...
				if (set_flags[fragidx])
					acc_mols[fragidx].reset(RDKit::combineMols(*acc_mols[fragidx], *mols[mol_idx]));
				else {
...
std::pair<int, int> FragmentTreeNode::findChargeLocation(RDKit::RWMol &rwmol, int charge_side, int radical_side,
                                                         bool is_negative) {
...
FragmentTreeNode::findAlreadyChargedOrSplitCharge(boost::tuple<int, int, int> &out_pidx_nidx_ridx, RDKit::RWMol &rwmol,
	...
	for (ai = rwmol.beginAtoms(); ai != rwmol.endAtoms(); ++ai) {
		...
		if ((*ai)->getNumRadicalElectrons() == 1)
			rad_idx =
			    (*ai)->getIdx(); // Note: only an assigned rad idx is added to out_pidx_nidx_ridx, this one is existing.
	}

	// std::cout <<"[DEBUG][findAlreadyChargedOrSplitCharge][charge_side]" << frag_total_qs[charge_side] <<
	// "[other_side]" << frag_total_qs[1 - charge_side] << std::endl;
	...
	if (!is_negative && frag_total_qs[charge_side] == 1 && frag_total_qs[1 - charge_side] == -1) {
		...
		for (; it != neg_idxs.end(); ++it) {
			std::unique_ptr<RDKit::Atom> neg_atom = std::unique_ptr<RDKit::Atom>(rwmol.getAtomWithIdx(*it));
			int nfragidx;
...
void FragmentTreeNode::undoAlreadyChargedOrSplitCharge(RDKit::RWMol &rwmol,
	...
	if (boost::get<2>(pidx_nidx_ridx) >= 0) {
		std::unique_ptr<RDKit::Atom> rad_atom =
		    std::unique_ptr<RDKit::Atom>(rwmol.getAtomWithIdx(boost::get<2>(pidx_nidx_ridx)));
	...
	if (boost::get<1>(pidx_nidx_ridx) >= 0) {
		std::unique_ptr<RDKit::Atom> neg_atom =
		    std::unique_ptr<RDKit::Atom>(rwmol.getAtomWithIdx(boost::get<1>(pidx_nidx_ridx)));
...
int FragmentTreeNode::findAtomChargeLocationNSOC(RDKit::RWMol &rwmol, int charge_side, bool is_negative) {
	...
	for (unsigned int i = 0; i < rwmol.getNumAtoms(); i++) {
		...
		if (on_nitro_group > 0) continue;

		unsigned int numrings;
		atom->getProp("NumUnbrokenRings", numrings);
		if (numrings > 0) {
			if (atom->getIsAromatic()) continue;
			if (atom->getSymbol() == "N" && NidxR < 0)
				NidxR = i;
			else if (atom->getSymbol() == "P" && PidxR < 0)
				PidxR = i;
			else if (atom->getSymbol() == "S" && SidxR < 0)
				SidxR = i;
			else if (atom->getSymbol() == "O" && OidxR < 0)
				OidxR = i;
			else if (atom->getSymbol() == "C" && CidxR < 0)
				CidxR = i;
		} else {
			if (atom->getSymbol() == "N" && Nidx < 0)
				Nidx = i;
			else if (atom->getSymbol() == "P" && Pidx < 0)
				Pidx = i;
			else if (atom->getSymbol() == "S" && Sidx < 0)
				Sidx = i;
			else if (atom->getSymbol() == "O" && Oidx < 0)
				Oidx = i;
			else if (Hidx < 0 && (atom->getSymbol() == "Cl" || atom->getSymbol() == "I" || atom->getSymbol() == "Br" ||
			                      atom->getSymbol() == "F"))
				Hidx = i;
			else if (atom->getSymbol() == "C" && Cidx < 0)
				Cidx = i;
		}
	}
	...
	if (Nidx >= 0)
		output = Nidx; // Non-Ring Nitrogen
	else if (Pidx >= 0)
		output = Pidx; // Non-Ring Phosphorus
	else if (Sidx >= 0)
		output = Sidx; // Non-Ring Sulfur
	else if (Oidx >= 0)
		output = Oidx; // Non-Ring Oxygen
	else if (Hidx >= 0)
		output = Hidx; // Non-Ring Halogen
	else if (Cidx >= 0)
		output = Cidx; // Non-Ring Carbon
	else if (NidxR >= 0)
		output = NidxR; // Non-aromatic ring Nitrogen
	else if (PidxR >= 0)
		output = PidxR; // Non-aromatic ring Phosphorus
	else if (SidxR >= 0)
		output = SidxR; // Non-aromatic ring Sulfur
	else if (OidxR >= 0)
		output = OidxR; // Non-aromatic ring Oxygen
	else if (CidxR >= 0)
		output = CidxR; // Non-aromatic ring Carbon
...
void FragmentTreeNode::assignChargeAndRadical(RDKit::RWMol &rwmol, int charge_idx, int radical_idx, bool is_negative) {
	...
	std::unique_ptr<RDKit::Atom> atom = std::unique_ptr<RDKit::Atom>(rwmol.getAtomWithIdx(charge_idx));
	...
	if (is_negative) {
		...
		if (radical_idx < 0)
			...
			std::unique_ptr<RDKit::Atom> rad_atom = std::unique_ptr<RDKit::Atom>(rwmol.getAtomWithIdx(radical_idx));
...
void FragmentTreeNode::labelBreakPropertiesInNL(romol_ptr_t &current_nl, romol_ptr_t &parent_ion, Break &brk) {
	...
	if (fh->getExecFlag(3)) {
		...
		if (brk.isIonicBreak())
			current_nl.get()->setProp("BrokenOrigBondType", 6);
		else if (brk.isHydrogenOnlyBreak())
			current_nl.get()->setProp("BrokenOrigBondType", 7);
		else {
...
void FragmentTreeNode::labelExtraBreakPropertiesInIon(romol_ptr_t &current_ion, romol_ptr_t &parent_ion, Break &brk) {
	...
	if (fh->getExecFlag(3)) {
		...
		if (parent_ion->hasProp(history_keyword)) { parent_ion->getProp(history_keyword, history_so_far); }
...
void FragmentTreeNode::generateBreaks(std::vector<Break> &breaks, bool include_H_only_loss, bool include_cyclization) {
	std::unique_ptr<RDKit::ROMol> _ion = std::unique_ptr<RDKit::ROMol>(ion.get());
	int num_ionic                      = countNumIonicFragments(_ion);
	...
	for (auto &atom : ion->atoms()) {
		atom->setProp("NumUnbrokenRings", rinfo->numAtomRings(atom->getIdx()));

		// Fetch or compute the valence of the atom in the input molecule (we disallow valence changes for now)
		auto orig_val = getValence(std::unique_ptr<RDKit::Atom>(atom));
		atom->setProp("OrigValence", orig_val);
		atom->setProp("Root", 0);
		atom->setProp("OtherRoot", 0);
		...
		if (ionic_frag_q != 0 &&
			...
			breaks.push_back(Break(atom->getIdx(), true, false, -1, computeNumIonicAlloc(num_ionic - 1), false));
	}
	...
	for (unsigned int bidx = 0; bidx < ion.get()->getNumBonds(); bidx++) {
		...
		if (had_ring_break) {
			...
			if (rinfo->numBondRings(bidx) == 0 && was_on_the_ring) bond->setProp("OnTheRing", 0);
		}

		// if this is a node right after ring break
		if (had_ring_break && !was_on_the_ring) continue;
	...
	if (had_ring_break == 0) {
		...
		for (int ring_idx = 0; bit != brings.end(); ++bit, ring_idx++) {
			...
			for (auto it = bit->begin(); it != bit->end(); ++it) {
				...
				if (rinfo->numBondRings(*it) != 1) continue;
...
void FragmentTreeNode::applyBreak(Break &brk, int ionic_allocation_idx) {
	...
	RDKit::ROMol::AtomIterator ai;
	for (ai = ion.get()->beginAtoms(); ai != ion.get()->endAtoms(); ++ai) (*ai)->setProp("FragIdx", 0);
...
void FragmentTreeNode::allocatedCtdToFragment(std::unique_ptr<RDKit::ROMol> &romol,
	...
	RDKit::ROMol::ADJ_ITER_PAIR itp = romol->getAtomNeighbors(atom.get());
	for (; itp.first != itp.second; ++itp.first) {
		std::unique_ptr<RDKit::Atom> nbr_atom = std::unique_ptr<RDKit::Atom>(romol->getAtomWithIdx(*itp.first));
...
void FragmentTreeNode::labelIonProperties() {
	std::unique_ptr<RDKit::ROMol> _ion = std::unique_ptr<RDKit::ROMol>(ion.get());
	int is_radical                     = moleculeHasSingleRadical(_ion);
...
int FragmentTreeNode::countNumIonicFragments(const std::unique_ptr<RDKit::ROMol> &romol) {
	int num_ionic = 0;
	for (auto &atoms : romol->atoms()) {
		int ionic_frag_q;
		atoms->getProp("IonicFragmentCharge", ionic_frag_q);
		if (ionic_frag_q != 0) num_ionic++;
	}
	return num_ionic;
}

int FragmentTreeNode::computeNumIonicAlloc(int num_ionic_fragments) { return 1 << num_ionic_fragments; }
