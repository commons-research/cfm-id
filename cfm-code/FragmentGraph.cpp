...
Transition::Transition(int a_from_id, int a_to_id, const romol_ptr_t &a_nl, const romol_ptr_t &an_ion) {
	...
	if (root == nullptr) std::cerr << "Warning: Ion Root atoms not defined" << std::endl;
	...
	if (root == nullptr) std::cerr << "Warning: NL Root atoms not defined" << std::endl;
...
int FragmentGraph::addToGraph(const FragmentTreeNode &node, int parentid) {
	...
	if (parentid < 0 || fragments[id]->getDepth() == -1)
		fragments[id]->setDepth(node.depth); // Set start fragment
		                                     // depth

	// Add a transition, IF:
	//  - This transition does not exist (i.e. this parent_id -> id)
	//  - There is no (already added) shorter path to this fragment (we track the minimum depth at which each fragment
	//  is seen)
...
int FragmentGraph::addToGraphAndReplaceMolWithFV(const FragmentTreeNode &node, int parent_frag_id,
	...
	if (parent_frag_id >= 0) { existing_trans_id = findMatchingTransition(parent_frag_id, frag_id); }
	...
	if (parent_frag_id >= 0 && existing_trans_id < 0 &&
		...
		auto frag_ptr           = createMolPtr(frag_smiles.c_str(), false);
		...
		t->deleteIon();
		t->deleteNeutralLoss();
	} else if (parent_frag_id >= 0 && existing_trans_id >= 0 && (node.depth == fragments[frag_id]->getDepth())) {
...
int FragmentGraph::addToGraphWithThetas(const FragmentTreeNode &node, const std::vector<double> *thetas,
                                        int parent_frag_id) {
...
int FragmentGraph::addFragmentOrFetchExistingId(romol_ptr_t ion, double mass, bool is_intermediate,
	...
	if (frag_mass_lookup.find(rounded_mass) != frag_mass_lookup.end()) {
		...
		for (; it != frag_mass_lookup[rounded_mass].end(); ++it) {
			try {
				RDKit::RWMol *f2_reduced = RDKit::SmilesToMol(*fragments[*it]->getReducedSmiles(), 0, false);
	...
	if (include_isotopes) {
		...
		fragments.push_back(
		    new Fragment(smiles, reduced_smiles, newid, mass, isotope_spectrum, is_intermediate, is_cyclization));
	} else {
...
bool FragmentGraph::areMatching(RDKit::ROMol *f1_reduced_ion, RDKit::ROMol *f2_reduced_ion) {
	...
	if (f1_reduced_ion->getNumAtoms() != f2_reduced_ion->getNumAtoms()) return false;
...
int FragmentGraph::findMatchingTransition(int from_id, int to_id) {
	...
	for (; it != from_id_tmap[from_id].end(); ++it) {
		if (*it >= transitions.size()) continue;
...
void FragmentGraph::writeFragmentsOnly(std::ostream &out) const {
	...
	for (auto &it : fragments) {
		...
		if (it->isIntermediate()) out << " Intermediate Fragment";
		if (it->isCyclization()) out << " Cyclization Fragment";
		out << std::endl;
	}
}
...
void FragmentGraph::readFeatureVectorGraph(std::istream &ifs) {
	...
	for (int i = 0; i < numt; i++) {
		...
		if (fv->getTotalLength() != fv_len) fv->addFeatureAtIdx(0.0, fv_len - 1);
...
void FragmentGraph::removeDetours() {
	...
	for (int i = 0; i < transitions.size(); i++) {
		...
		if (fragments[from_id]->getDepth() >= fragments[to_id]->getDepth()) remove_ids.push_back(i);
	}
	...
	for (int i = 0; i < transitions.size(); i++) {
		...
		if (need_remove) need_remove = (need_remove && (remove_ids[id_idx] == i));
		// if deleting
		if (need_remove) {
			id_map[i] = -1;
			++id_idx;
	...
	for (int i = 0; i < fragments.size(); i++) {
		...
		for (int j = 0; j < to_id_tmap[i].size(); j++) {
			if (id_map[to_id_tmap[i][j]] >= 0) to_id_tmap[i][count++] = id_map[to_id_tmap[i][j]];
		}
		...
		for (int j = 0; j < from_id_tmap[i].size(); j++) {
			if (id_map[from_id_tmap[i][j]] >= 0) from_id_tmap[i][count++] = id_map[from_id_tmap[i][j]];
		}
		from_id_tmap[i].resize(count);
	}
}

void FragmentGraph::getSampledTransitionIdsRandomWalk(std::set<int> &selected_ids, int max_selection) {
...
void FragmentGraph::getSampledTransitionIdsWeightedRandomWalk(std::set<int> &selected_ids, int max_num_iter,
	...
	for (auto &frag_trans_ids : from_id_tmap) {
		...
		for (auto &trans_id : frag_trans_ids) { weights.push_back(thetas[trans_id]); }
...
void FragmentGraph::getSampledTransitionIdsDiffMapChildOnly(std::set<int> &selected_ids,
                                                            std::set<unsigned int> &selected_weights) {
	std::set<int> visited;
	std::vector<int> path;
	getSampledTransitionIdsWeightDiffChildOnly(selected_weights, visited, 0, path, selected_ids);
}

void FragmentGraph::getSampledTransitionIdsDiffMap(std::set<int> &selected_ids,
                                                   std::set<unsigned int> &selected_weights) {
	std::set<int> visited;
	...
	for (const auto &record : frag_trans_map) {
		if (record.second.size() > 1)
			for (const auto &trans_id : record.second) selected_ids.insert(trans_id);
	}
}

void FragmentGraph::getSampledTransitionIdsWeightDiffs(std::set<unsigned int> &selected_weights, std::set<int> &visited,
	...
	if (visited.find(frag_id) != visited.end()) return;
	...
	for (const auto &trans_id : from_id_tmap[frag_id]) {
		...
		getSampledTransitionIdsWeightDiffs(selected_weights, visited, transitions[trans_id]->getToId(), current_path,
		                                   trans_to_interest_frags_map);
	}
}

void FragmentGraph::getSampledTransitionIdsWeightDiffChildOnly(std::set<unsigned int> &selected_weights,
	...
	for (const auto &trans_id : from_id_tmap[frag_id]) {
		...
		if (is_match(selected_weights, child_frag_mass)) matched_selected_ids.push_back(trans_id);

		getSampledTransitionIdsWeightDiffChildOnly(selected_weights, visited, child_frag_id, path_to_current_child,
		                                           selected_ids);
	}
	...
	if (!matched_selected_ids.empty()) {
		...
		for (const auto &trans_id : path) selected_ids.insert(trans_id);

		// add all trans in this node
		for (const auto &trans_id : matched_selected_ids) selected_ids.insert(trans_id);
	}
}
...
void FragmentGraph::getRandomSampledTransitions(std::set<int> &selected_trans_id, int max_selection) {
	...
	for (int i = 0; i < max_selection; ++i) selected_trans_id.insert(ids[i]);
}

void FragmentGraph::clearAllSmiles() {
	for (auto &fragment : fragments) { fragment->clearSmiles(); }
};
...
bool EvidenceFragmentGraph::fragmentIsRedundant(unsigned int fidx, std::vector<int> &annotated_flags,
	...
	for (; it != from_id_tmap[fidx].end(); ++it) {
		...
		if (!fragmentIsRedundant(cidx, annotated_flags, direct_flags, visited) && !direct_flags[cidx]) return false;
	}
	return true;
}
