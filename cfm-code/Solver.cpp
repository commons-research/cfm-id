...
void Sgd::adjustWeights(std::vector<float> &grads, std::set<unsigned int> &used_idxs, boost::shared_ptr<Param> &param) {

	for (auto &used_idx : used_idxs) param->getWeights()[used_idx] += learning_rate * grads[used_idx];
}
...
void Momentum::adjustWeights(std::vector<float> &grads, std::set<unsigned int> &used_idxs,
                             boost::shared_ptr<Param> &param) {

	for (auto &used_idx : used_idxs) {
		float v = momentum * prev_v[used_idx] + learning_rate * grads[used_idx];
		param->getWeights()[used_idx] += v;
		prev_v[used_idx] = v;
	}
}

Adam::Adam(unsigned int length, float learning_rate, float beta_1, float beta_2, float eps, bool use_amsgrad) {
	...
	if (this->use_amsgrad) this->second_moment_max_vector = std::vector<float>(length, 0.0);
}

void Adam::adjustWeights(std::vector<float> &grads, std::set<unsigned int> &used_idxs,
                         boost::shared_ptr<Param> &param) {
	...
	for (auto &used_idx : used_idxs) {
		...
		float v_t = beta_2 * second_moment_vector[used_idx] + (1.0 - beta_2) * std::pow(grads[used_idx], 2);
		...
		param->getWeights()[used_idx] += learning_rate * m_hat / (std::sqrt(v_hat) + eps);
	}
}

void AdamW::adjustWeights(std::vector<float> &grads, std::set<unsigned int> &used_idxs,
                          boost::shared_ptr<Param> &param) {
	...
	for (auto &used_idx : used_idxs) {
		...
		if (std::find(bias_index.begin(), bias_index.end(), used_idx) != bias_index.end())
			param->getWeights()[used_idx] =
			    (1.0 - w) * param->getWeights()[used_idx] + learning_rate * m_hat / (std::sqrt(v_hat) + eps);
		else
			param->getWeights()[used_idx] =
			    param->getWeights()[used_idx] + learning_rate * m_hat / (std::sqrt(v_hat) + eps);
	}
...
AdaDelta::AdaDelta(unsigned int length, float learning_rate, float decay_rate, float eps) {
...
void AdaDelta::adjustWeights(std::vector<float> &grads, std::set<unsigned int> &used_idxs,
                             boost::shared_ptr<Param> &param) {
	...
	for (auto &used_idx : used_idxs) {
		...
		param->getWeights()[used_idx] -= detla_x;
	}
}

void AdaBelief::adjustWeights(std::vector<float> &grads, std::set<unsigned int> &used_idxs,
                              boost::shared_ptr<Param> &param) {
	...
	for (auto &used_idx : used_idxs) {
		...
		float s_t = beta_2 * second_moment_vector[used_idx] + (1.0 - beta_2) * std::pow(g_t - m_t, 2);
		...
		// Update parameters
		// theta_t = theta_{t-1} - alpha * m_hat / ( sqrt(v_hat) + eps)
		param->getWeights()[used_idx] += learning_rate * m_t / (std::sqrt(s_hat) + eps);
	}
}
