...
void RootPathFeature::computeRootPaths(std::vector<path_t> &paths, const std::unique_ptr<RootedROMol> &mol, int len,
                                       bool with_bond = false) const {
	path_t path_so_far;
	addPathsFromAtom(paths, mol->root, mol->mol, mol->root, path_so_far, len, with_bond);
}

void RootPathFeature::addPathsFromAtom(std::vector<path_t> &paths, const std::unique_ptr<RDKit::Atom> &atom,
                                       romol_ptr_t &mol, const std::unique_ptr<RDKit::Atom> &prev_atom,
	...
	if (len > 1) {
		RDKit::ROMol::ADJ_ITER_PAIR itp = mol.get()->getAtomNeighbors(atom.get());
		for (; itp.first != itp.second; ++itp.first) {
			std::unique_ptr<RDKit::Atom> nbr_atom =
			    std::make_unique<RDKit::Atom>(*mol.get()->getAtomWithIdx(*itp.first));
			if (with_bond == true) {
				int bond_type = 0;
...
void RootPathFeature::addRootFeaturesWithBond(FeatureVector &fv, std::vector<path_t> &paths, int ring_break,
                                              int len) const {
	// double count = 0.0, ring_count = 0.0;
