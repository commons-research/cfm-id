...
void RootPathFeature::computeRootPaths(std::vector<path_t> &paths, const RootedROMol *mol, int len,
	...
	path_t path_so_far;
	addPathsFromAtom(paths, mol->_root, mol->_mol, mol->_root, path_so_far, len, with_bond);
}

void RootPathFeature::addPathsFromAtom(std::vector<path_t> &paths, const RDKit::Atom &atom, const romol_ptr_t &mol,
                                       const RDKit::Atom &prev_atom, path_t &path_so_far, int len,
	...
	if (len > 1) {
		...
		for (; itp.first != itp.second; ++itp.first) {
			const auto *nbr_atom = mol.get()->getAtomWithIdx(*itp.first);
			if (with_bond == true) {
				int bond_type = 0;
...
void RootPathFeature::addRootPairFeatures(FeatureVector &fv, std::vector<path_t> &paths, int ring_break) {
	...
	for (it1 = ok_symbols->begin(); it1 != ok_symbols->end(); ++it1) {
		for (it2 = ok_symbols->begin(); it2 != ok_symbols->end(); ++it2) {
			...
			sp.push_back(*it1);
			sp.push_back(*it2);
			auto it3     = paths.begin();
			double count = 0.0, ring_count = 0.0;
			for (; it3 != paths.end(); ++it3) {
				if (!ring_break && sp[0] == (*it3)[0] && sp[1] == (*it3)[1]) count += 1.0;
				if (ring_break && sp[0] == (*it3)[0] && sp[1] == (*it3)[1]) ring_count += 1.0;
			}
...
void RootPathFeature::addRootTripleFeatures(FeatureVector &fv, std::vector<path_t> &paths, int ring_break) {
...
void RootPathFeature::addRootFeaturesWithBond(FeatureVector &fv, std::vector<path_t> &paths, int ring_break, int len) {
	// double count = 0.0, ring_count = 0.0;
	size_t num_symbol_type        = OKSymbolsLess().size();
	unsigned int base_feature_idx = fv.getTotalLength();
	std::unordered_map<size_t, int> counts;
	// for each path in the path list
	for (auto &path : paths) {
		size_t feature_idx_offset = 0;
		for (unsigned int idx = 0; idx < path.size(); ++idx) {
			const std::string &symbol = path.at(idx);
			...
			if (idx % 2 == 1) {
				...
				feature_idx_offset = num_symbol_type * feature_idx_offset + getSymbolsIndex(symbol, false);
			}
		}

		size_t key = ring_break ? feature_idx_offset * 4 : feature_idx_offset * 4 + 2;

		if (counts.find(key) == counts.end()) { counts[key] = 0; }
		counts[key] += 1;

		if (counts[key] > 1) { key += 1; }
