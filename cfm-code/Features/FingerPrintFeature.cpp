...
void FingerPrintFeature::getRemoveAtomIdxByDisatnce(romol_ptr_t mol, const RDKit::Atom *root,
	...
	while (!atom_queue.empty() && !distance_queue.empty()) {
		...
		// tracking cycles
		if (visited.find(curr->getIdx()) != visited.end()) { continue; }

		visited.insert(curr->getIdx());

		// thus we need remove it
		// also we does not care Hs
		if (distance < curr_distance || curr->getSymbol() == "H") { remove_atom_ids.push_back(curr->getIdx()); }
...
void FingerPrintFeature::getRemoveAtomIdxByCount(romol_ptr_t mol, const RDKit::Atom *root,
                                                 std::vector<unsigned int> &remove_atom_ids, int count) const {
	std::queue<const RDKit::Atom *> atom_queue;
	std::queue<int> distance_queue;
	atom_queue.push(root);
	distance_queue.push(0);
	...
	while (!atom_queue.empty() && !distance_queue.empty()) {
		...
		// thus we need remove it
		// also we does not care Hs
		if (visited.size() >= count) { remove_atom_ids.push_back(curr->getIdx()); }
...
void FingerPrintFeature::removeAtomInTheList(RDKit::RWMol &mol, std::vector<unsigned int> &remove_atom_ids) const {

	// remove all duplications in the list
	// in theory this should not happen
	remove_atom_ids.erase(unique(remove_atom_ids.begin(), remove_atom_ids.end()), remove_atom_ids.end());
	...
	// remove ops
	std::sort(remove_atom_ids.begin(), remove_atom_ids.end(), std::greater<int>());

	for (auto atom_idx : remove_atom_ids) { mol.removeAtom(atom_idx); }
}

void FingerPrintFeature::addRDKitFingerPrint(std::vector<int> &tmp_fv, const std::unique_ptr<RootedROMol> &mol,
                                             const RDKit::Atom *root, unsigned int finger_print_size,
                                             unsigned int limitation_param, unsigned int finger_print_min_path,
                                             unsigned int finger_print_max_path, bool limited_by_distance) const {
	...
	ExplicitBitVect *finger_print =
	    RDKit::RDKFingerprintMol(part, finger_print_min_path, finger_print_max_path, finger_print_size);

	tmp_fv.resize(finger_print->getNumBits());
	for (unsigned int i = 0; i < finger_print->getNumBits(); ++i) tmp_fv[i] = (*finger_print)[i];

	delete finger_print;
}

void FingerPrintFeature::addRDKitFingerPrintFeatures(FeatureVector &fv, const std::unique_ptr<RootedROMol> &mol,
...
void FingerPrintFeature::addMorganFingerPrint(std::vector<int> &tmp_fv, const RootedROMol *mol, const RDKit::Atom *root,
	...
	// Get finger prints with size
	ExplicitBitVect *finger_print = RDKit::MorganFingerprints::getFingerprintAsBitVect(part, radius, finger_print_size);
...
void FingerPrintFeature::addMorganFingerPrintFeatures(FeatureVector &fv, const RootedROMol *mol,
                                                      unsigned int finger_print_size, unsigned int path_range,
                                                      int radius) const {

	std::vector<int> local_tmp_fv;
	addMorganFingerPrint(local_tmp_fv, mol, mol->root, finger_print_size, path_range, radius);
...
void FingerPrintFeature::addMorganFingerPrintFeatures(FeatureVector &fv, const RootedROMol *mol,
	...
	// Get finger prints with size
	ExplicitBitVect *finger_print =
	    RDKit::MorganFingerprints::getFingerprintAsBitVect((*mol->mol), radius, finger_print_size);

	local_tmp_fv.resize(finger_print->getNumBits());
	for (unsigned int i = 0; i < finger_print->getNumBits(); ++i) local_tmp_fv[i] = (*finger_print)[i];
...
std::string FingerPrintFeature::getSortingLabel(const romol_ptr_t mol, const RDKit::Atom *atom,
	...
	if (distance_to_root > 0) {
		...
		for (auto itp = mol->getAtomNeighbors(atom); itp.first != itp.second; ++itp.first) {
			...
			int bond_int =
			    FeatureHelper::getBondTypeAsInt(mol->getBondBetweenAtoms(atom->getIdx(), nbr_atom->getIdx()));

			// use smallest bond order
			if (child_distance_to_root < distance_to_root && bond_int < bond_type) bond_type = bond_int;
		}
	...
	if (symbol_str.size() == 1) symbol_str += " ";
	atom_key += symbol_str;
	...
	for (const auto child_key : children_keys) children_atom_key += "[" + child_key + "]";
	// save the label
...
void FingerPrintFeature::getAtomDistanceToRoot(const RootedROMol *roMolPtr,
	...
	while (!atom_queue.empty()) {
		...
		if (distances.find(curr->getIdx()) != distances.end()) continue;
...
void FingerPrintFeature::getBondAtomPairAtEachDistance(const RootedROMol *roMolPtr,
	...
	while (!atom_queue.empty()) {
		...
		for (auto itp = mol->getAtomNeighbors(curr); itp.first != itp.second; ++itp.first) {
			...
			if (curr_distance < dict.size()) dict[curr_distance][key] += 1;
		}
	}
}
...
void FingerPrintFeature::getAtomVisitOrderBFS(const RootedROMol *roMolPtr, std::vector<unsigned int> &visit_order,
	...
	while (!atom_queue.empty()) {
		...
		// if I have see this before
		if (std::find(visit_order.begin(), visit_order.end(), curr->getIdx()) != visit_order.end()) { continue; }
		...
		for (auto child : child_visit_order) atom_queue.push(child.second);
	}
}

void FingerPrintFeature::addAdjacentMatrixRepresentation(std::vector<int> &tmp_fv, const RootedROMol *roMolPtr,
...
void FingerPrintFeature::addGenernalizedRepresentation(std::vector<int> &tmp_fv, const RootedROMol *roMolPtr,
	...
	for (auto &symbol : symbols) {
		for (int bond_type = 1; bond_type <= num_bond_type; ++bond_type) {
			...
			for (int i = 0; i < max_distance; ++i) dicts[i][key] = 0;
		}
	}
	...
	for (int i = 0; i < offset; ++i) addBondAtomPairToFeatures(tmp_fv, dicts[i], use_full_symbols_set);
	// addBondAtomPairToFeatures(tmp_fv, dicts[1], false);
	for (int i = offset; i < max_distance; ++i) addBondAtomPairToFeatures(tmp_fv, dicts[i], use_full_symbols_set);
}

void FingerPrintFeature::addAdjMatrixFeatures(std::vector<int> &tmp_fv, const RootedROMol *mol, unsigned int num_atom,
                                              std::vector<unsigned int> &visit_order,
                                              std::vector<unsigned int> &distance, int min_distance, int max_distance,
                                              bool no_bond_type) const { // init a 2D vector to store matrix

	std::vector<std::vector<int>> adjacency_matrix(num_atom, std::vector<int>(num_atom, 0));
...
void FingerPrintFeature::getAdjMatrix(
    const RootedROMol *mol, unsigned int num_atom, const std::vector<unsigned int> &visit_order,
    std::vector<std::vector<int>> &adjacency_matrix, std::vector<unsigned int> &distance, int min_distance,
    int max_distance) const { // make sure we only get num_atom amount of atoms, this is extra check,
	...
	for (int i = 0; i < visit_order.size() && i < num_atom; ++i) {
		if ((distance[i] >= min_distance) && (distance[i] <= max_distance)) visit_order_map[visit_order[i]] = i;
	}
	...
	for (auto bi = mol->mol->beginBonds(); bi != mol->mol->endBonds(); ++bi) {
		...
		if (visit_order_map.find(begin_idx) != visit_order_map.end() &&
			...
			adjacency_matrix[visit_order_map[begin_idx]][visit_order_map[end_idx]] = bond_type;
			adjacency_matrix[visit_order_map[end_idx]][visit_order_map[begin_idx]] = bond_type;
		}
	}
}
...
void FingerPrintFeature::addAdjacentMatrixRepresentationFeature(FeatureVector &fv,
                                                                const std::unique_ptr<RootedROMol> &mol,
	...
	addAdjacentMatrixRepresentation(local_tmp_fv, mol, num_atom, max_distance, include_adjacency_matrix,
	                                use_full_symbols_set);
	fv.addFeatures(local_tmp_fv);
}

void FingerPrintFeature::addGenernalizedRepresentationFeature(FeatureVector &fv,
                                                              const std::unique_ptr<RootedROMol> &mol,
