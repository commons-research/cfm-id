...
void FeatureHelper::initialiseRoots(RDKit::RWMol *rwmol) {
	...
	for (ai = rwmol->beginAtoms(); ai != rwmol->endAtoms(); ++ai) (*ai)->setProp("Root", 0);
}
...
void FeatureHelper::labelFunctionalGroups(RDKit::RWMol *rwmol, bool extra) {
	...
	for (ai = rwmol->beginAtoms(); ai != rwmol->endAtoms(); ++ai) {
		...
		if (atom_fg_idxs[(*ai)->getIdx()].empty()) atom_fg_idxs[(*ai)->getIdx()].push_back(num_grps);
...
void FeatureHelper::labelAtomsWithLonePairs(RDKit::RWMol *rwmol) {
	...
	for (ai = rwmol->beginAtoms(); ai != rwmol->endAtoms(); ++ai) {
		...
		(*ai)->setProp(
		    "HasLP",
		    (int)(nouter > def_val && def_val != 1 && def_val != -1)); // Allow O,N,S,P..but not C, Halogens, Metals,.
	}
}
