...
void FunctionalGroupFeature::addFunctionalGroupFeatures(FeatureVector &fv, const std::unique_ptr<RootedROMol> &mol,
                                                        int max_depth, bool extra) const {
	...
	for (int idx = 0; it != tmp_full_fv.end(); ++it, idx++) fv.addFeatureAtIdx((double)(*it), offset + idx);
}

void FunctionalGroupFeature::addFunctionalGroupFeaturesFromAtom(std::vector<int> &tmp_full_fv,
                                                                const std::unique_ptr<RDKit::Atom> &atom,
                                                                const romol_ptr_t &mol,
                                                                const std::unique_ptr<RDKit::Atom> &prev_atom,
