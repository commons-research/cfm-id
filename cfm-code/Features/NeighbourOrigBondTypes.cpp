...
void NeighbourOrigBondTypes::compute(FeatureVector &fv, const std::unique_ptr<RootedROMol> &ion,
...
void addNeighbourOrigBondFeatures(FeatureVector &fv, const std::unique_ptr<RootedROMol> &mol, int ring_break) {

	std::vector<int> seen_types(6, 0);
	int feature_offset = fv.getTotalLength();
	// RDKit::ROMol::ADJ_ITER_PAIR
	auto itp           = mol->mol->getAtomNeighbors(mol->root.get());
