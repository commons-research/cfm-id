...
void NeighbourMMFFFeature::addNeighbourAtomTypes(FeatureVector &fv, const std::unique_ptr<RootedROMol> &mol,
                                                 const std::unique_ptr<RDKit::Atom> &root, int offset) const {
	// Iterate over the neighbours of the root atom
	RDKit::ROMol::ADJ_ITER_PAIR itp = mol->mol->getAtomNeighbors(root.get());
	int num_added                   = 0;
	for (; itp.first != itp.second; ++itp.first) {
		...
		if (atomtype < 1 || atomtype > 99) fv.addFeatureAtIdx(1.0, offset + 100);
		num_added++;
	}
