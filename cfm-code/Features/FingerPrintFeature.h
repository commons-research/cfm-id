...
protected:
	...
	void getRemoveAtomIdxByCount(romol_ptr_t mol, const RDKit::Atom *root, std::vector<unsigned int> &remove_atom_ids,
	                             int count) const;
...
private:
	...
	void addMorganFingerPrint(std::vector<int> &tmp_fv, const RootedROMol *mol, const RDKit::Atom *root,
	...
	void addAdjacentMatrixRepresentation(std::vector<int> &tmp_fv, const RootedROMol *roMolPtr, unsigned int num_atom,
	                                     unsigned int depth, bool include_adjacency_matrix,
	                                     bool use_full_symbols_set) const;
	...
	void addAtomTypeSeqFeatures(std::vector<int> &tmp_fv, const RootedROMol *mol, unsigned int num_atom,
	                            ...
	                            int min_distance, int max_distance, bool use_full_okay_symbol_set) const;
	...
	void addDistanceFeature(std::vector<int> &tmp_fv, unsigned int num_atom,
	                        const std::vector<unsigned int> &distance) const;

	void addBondAtomPairToFeatures(std::vector<int> &tmp_fv, std::map<std::string, int> &dict, bool no_count) const;

	void updateBondAtomPairDict(const RootedROMol *rootedMol, const RDKit::Atom *root, std::map<std::string, int> &dict,
	                            bool use_full_symbols_set) const;

	void getAdjMatrix(const RootedROMol *mol, unsigned int num_atom, const std::vector<unsigned int> &visit_order,
	                  std::vector<std::vector<int>> &adjacency_matrix, std::vector<unsigned int> &distance,
	                  int min_distance, int max_distance) const;

	void getBondAtomPairAtEachDistance(const RootedROMol *roMolPtr, std::vector<std::map<std::string, int>> &dict,
	                                   bool use_full_symbols_set) const;
};
