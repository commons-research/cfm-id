...
protected:
	...
	void computeRootPaths(std::vector<path_t> &paths, const std::unique_ptr<RootedROMol> &mol, int len,
	...
	void addRootFeaturesWithBond(FeatureVector &fv, std::vector<path_t> &paths, int ring_break, int len) const;

private:
	// function to add path from given atom
	void addPathsFromAtom(std::vector<path_t> &paths, const std::unique_ptr<RDKit::Atom> &atom, romol_ptr_t &mol,
	                      const std::unique_ptr<RDKit::Atom> &prev_atom, path_t &path_so_far, int len,
	                      bool with_bond) const;
};
