	...
	while (ifs.good()) {
		...
		if (numpeaks > 0) {
			if (line.substr(0, 3) == "ID:" || line.substr(0, 5) == "Name:") {
				...
				std::cout << "Warning: Number of peaks read did not match stated number of peaks for " << id
				          << std::endl;
				numpeaks = 0;
			} else {
				...
				for (; tok_iter != tokens.end(); ++tok_iter) {
					...
					if (numpeaks == 1) {
						if (normalize_and_sort) library[id][energy].normalizeAndSort();
						if (quantise_dec_pl >= 0) library[id][energy].quantisePeaksByMass(quantise_dec_pl);
					}
					numpeaks--;
				}
			}
		...
		if (id == "") {
			if (boost::to_upper_copy(line.substr(0, 9)) == "INCHIKEY:") {
				std::stringstream ss1(line);
				std::string tag;
				ss1 >> tag >> id;
				id = pre_id + id;
		...
		if (boost::to_upper_copy(line.substr(0, 10)) == "NUM PEAKS:") {
			tokenizer tokens(line, sep);
			tokenizer::iterator tok_iter = tokens.begin();
			if (++tok_iter != tokens.end()) numpeaks = atoi(tok_iter->c_str());
			if (id == "") {
			...
			if (library[id].size() < energy + 1) library[id].resize(energy + 1);
			num_added += 1;
		}
	}
