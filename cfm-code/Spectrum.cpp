...
void Spectrum::outputToMspStream(std::ostream &out, std::string id, int ionization_mode, int energy,
	...
	out << "Num peaks: " << this->_peaks.size() << std::endl;
...
void Spectrum::postProcess(double perc_thresh, int min_peaks, int max_peaks, double min_relative_intensity_prec) {
	...
	if (_peaks.empty()) return;

	std::sort(_peaks.begin(), _peaks.end(), sort_peaks_by_intensity);
	auto max_intensity = _peaks[0].intensity();
	double total       = 0.0;
	int count          = 0;
	...
	_peaks.resize(count);
	std::sort(_peaks.begin(), _peaks.end(), sort_peaks_by_mass);
}
...
void Spectrum::normalizeAndSort() {
	...
	if (!_is_normalized) {
		...
		double sum = 0.0;
		auto itp   = _peaks.begin();
		for (; itp != _peaks.end(); ++itp) sum += itp->intensity();
		double norm = 1.0;
		if (sum > 0.0) norm = 100.0 / sum;

		// Adjust the values
		for (itp = _peaks.begin(); itp != _peaks.end(); ++itp) itp->new_intensity(itp->intensity() * norm);
	}

	// Ensure the peaks are sorted by mass
	if (!_is_sorted) std::sort(_peaks.begin(), _peaks.end(), sort_peaks_by_mass);
...
void Spectrum::clean(double abs_mass_tol, double ppm_mass_tol) {
	...
	std::vector<bool> peak_flags(_peaks.size(), true);

	// Forward Pass
	double prev_intensity = -1.0;
	double prev_mass      = -100.0;
	auto itp              = _peaks.begin();
	for (int idx = 0; itp != _peaks.end(); ++itp, idx++) {
	...
	prev_intensity = -1.0;
	prev_mass      = -100.0;
	auto ritp      = _peaks.rbegin();
	for (size_t idx = _peaks.size() - 1; ritp != _peaks.rend(); ++ritp, idx--) {
	...
	std::vector<Peak> peaks_copy(_peaks);
	_peaks.clear();
	itp = peaks_copy.begin();
	for (int idx = 0; itp != peaks_copy.end(); ++itp, idx++)
		if (peak_flags[idx]) _peaks.push_back(*itp);
...
void Spectrum::convertToLogScale() {
	...
	for (auto &peak : _peaks) max_intensity = std::max(peak.intensity(), max_intensity);

	// convert peak height to relative to main peak (height 100.0)
	// make sure we don't have negative value
	for (auto &peak : _peaks) peak.new_intensity((log(std::max(peak.intensity() / max_intensity * 100.0, 1.0))));
	normalizeAndSort();
}
...
void Spectrum::convertToLinearScale() {
	...
	for (auto &peak : _peaks) peak.new_intensity(exp((peak.intensity() / max_intensity) / ratio) - 1);
	normalizeAndSort();
}
