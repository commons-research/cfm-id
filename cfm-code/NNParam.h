...
class NNParamActivationFunctionIdException : public std::exception {
	...
	virtual const char *what() const noexcept { return "Unknown activation function"; }
};
...
public:
	...
	explicit NNParam(std::string &filename);
	...
	float computeTheta(const FeatureVector &fv, int energy) override;
	...
	void computeDeltas(std::vector<azd_vals_t> &deltasA, std::vector<azd_vals_t> &deltasB,
	                   std::vector<azd_vals_t> &z_values, std::vector<azd_vals_t> &a_values, float rho_denom,
	                   int energy);

	void computeUnweightedGradients(std::vector<std::vector<float>> &unweighted_grads,
	...
	virtual std::unique_ptr<boost::container::vector<bool>> getDropoutsPtr() override {
		return std::make_unique<boost::container::vector<bool>>(hlayer_is_dropped);
	};

	virtual std::unique_ptr<std::vector<float>> getDropoutsProbPtr() override {
		return std::make_unique<std::vector<float>>(hlayer_dropout_probs);
	};
	;

	void rollDropouts() override;

	void collectUsedIdx(std::set<unsigned int> &used_idxs, unsigned int feature_len, unsigned offset, unsigned fv_idx,
	                    unsigned int energy) {
...
private:
	...
	// ReLU Activation
	static float relu_activation(float input) {
		if (input > 0)
			return input;
		else
			return 0;
	...
	static float leaky_relu_activation(float input) {
		if (input > 0)
			return input;
		else
			return input * 0.01;
	...
	static float leaky_relu_derivative(float input) {
		if (input > 0)
			return 1;
		else
			return 0.01;
	...
	static float neg_relu_activation(float input) {
		if (input < 0)
			return input;
		else
			return 0;
	...
	static float neg_relu_derivative(float input) {
		if (input < 0)
			return 1;
		else
			return 0;
