...
public:
	...
	Break()
	    : h_only_break(true), ring_break(false), ionic_break(false), bond_idx(-1), ring_idx(-1),
	      num_ionic_frag_allocations(1), ionic_idx(-1), is_cycliaztion(false) {};

	// Constructor for Break (ionic or standard or ring)
	Break(int a_bond_or_ionic_atom_idx, bool a_ionic_break, bool is_ring_break, int a_ring_idx,
	      int a_num_ionic_frag_allocations, bool is_cycliaztion)
	    : h_only_break(false), ring_break(is_ring_break), ionic_break(a_ionic_break), bond_idx(-1), ring_idx(-1),
	      ionic_idx(-1), num_ionic_frag_allocations(a_num_ionic_frag_allocations), is_cycliaztion(is_cycliaztion) {

		if (is_ring_break) ring_idx = a_ring_idx;
		if (ionic_break)
...
public:
	// Constructors
	explicit FragmentTreeNode(std::unique_ptr<FeatureHelper> a_fh) : fh(std::move(a_fh)) {};
	...
	FragmentTreeNode(romol_ptr_t an_ion, romol_ptr_t a_nl, int a_ion_free_ep, int a_depth, FeatureHelper *a_fh,
	    ...
	    : ion(std::move(an_ion)), nl(std::move(a_nl)), ion_free_epairs(a_ion_free_ep), depth(a_depth), fh(a_fh),
	      e_loc(a_e_loc), is_intermediate(is_intermediate) {
		labelIonProperties();
	};

	// used for create start node
	FragmentTreeNode(romol_ptr_t an_ion, int a_ion_free_ep, int a_depth, std::unique_ptr<FeatureHelper> a_fh,
	                 std::vector<int> &a_e_loc)
	    : ion(std::move(an_ion)), ion_free_epairs(a_ion_free_ep), depth(a_depth), fh(std::move(a_fh)), e_loc(a_e_loc),
	...
	static std::pair<int, int> findChargeLocation(RDKit::RWMol &rwmol, int charge_side, int radical_side,
	                                              bool is_negative);

	static boost::tuple<bool, bool, bool> findAlreadyChargedOrSplitCharge(boost::tuple<int, int, int> &pidx_nidx_ridx,
...
private:
	...
	// computation
	std::unique_ptr<FeatureHelper> fh;
	...
	void allocatedCtdToFragment(std::unique_ptr<RDKit::ROMol> &romol, std::unique_ptr<RDKit::Atom> &atom);
	...
	int addChild(int e_f0, int e_to_allocate, std::vector<int> &output_bmax, Break &brk, int charge_frag);
	...
	// neutral loss
	void labelBreakPropertiesInNL(romol_ptr_t &current_nl, romol_ptr_t &parent_ion, Break &brk);

	// Utility function to store extra  properties of the break in the generated iom
	void labelExtraBreakPropertiesInIon(romol_ptr_t &current_ion, romol_ptr_t &parent_ion, Break &brk);
	...
	static int countNumIonicFragments(
	    const std::unique_ptr<RDKit::ROMol> &romol); // Requires IonicFragmentCharge labels to be present
