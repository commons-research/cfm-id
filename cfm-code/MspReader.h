...
class MspReadException : public std::exception {
	...
	virtual const char *what() const noexcept { return "Could not read msp file."; }
};

class MspIdException : public std::exception {

	virtual const char *what() const noexcept { return "Could not find id in msp file."; }
};
...
public:
	...
	explicit MspReader(const char *filename, const char *pre_id = "", bool normalize_and_sort = false,
	                   int quantise_dec_pl = -1) {
		readInMspFile(filename, pre_id, normalize_and_sort, quantise_dec_pl);
	};
	...
	void writeLibraryToMspFile(const char *filename, std::string &smiles_or_inchi,
	                           int ionization_mode = DEFAULT_IONIZATION_MODE) const;
