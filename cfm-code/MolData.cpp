...
void MolData::computeGraphWithGenerator(FragmentGraphGenerator &fgen) {
  ...
  try {
    ...
    std::cerr << "Error Processing " << smiles_or_inchi << " " << e.what()
              << std::endl;
...
void MolData::computeEvidenceFragmentGraph(beliefs_t *beliefs,
  ...
  id_lookup[0] = ev_fg->addToGraphDirectNoCheck(
      EvidenceFragment(*(fg->getFragmentAtIdx(0)), -1, main_ev), &null_trans,
      -1);
...
void MolData::computeTransitionThetas(Param &param) {
  ...
  for (unsigned int energy = 0; energy < num_levels; energy++) {
    ...
    for (unsigned int i = 0; i < fg->getNumTransitions(); i++)
      ...
      thetas[energy][i] = param.computeTheta(
          *(fg->getTransitionAtIdx(i)->getFeatureVector()), energy);
  }
}
...
void MolData::computeLogTransitionProbabilities() {
  ...
  for (unsigned int energy = 0; energy < thetas.size(); energy++) {
    ...
    for (unsigned int i = 0; i < fg->getNumFragments(); i++) {
      double denom = 1.0;
      for (auto trans_idx = (*from_id_map)[i].begin();
           trans_idx != (*from_id_map)[i].end(); ++trans_idx) {
...
std::string MolData::removePeaksWithNoFragment(double abs_tol, double ppm_tol) {
  ...
  for (auto &spectrum : spectra) {
    ...
    msg += " Spectrum#" + std::to_string(spectrum_idx) +
...
void MolData::computePredictedSpectra(Param &param, bool use_existing_thetas,
                                      int energy_level, int min_peaks,
                                      int max_peaks, double perc_thresh,
                                      double min_relative_intensity,
                                      int quantise_peaks_decimal_place,
                                      bool log_to_linear) {
  ...
  postprocessPredictedSpectra(perc_thresh, min_peaks, max_peaks,
...
void MolData::createSpeactraSingleEnergry(unsigned int energy_level) {
  ...
  int depth = getFGHeight() > se_cfg.spectrum_depths[0]
                  ? getFGHeight()
                  : se_cfg.spectrum_depths[0];
...
void MolData::writePredictedSpectraToMspFileStream(std::ostream &out) {
  for (unsigned int energy = 0; energy < getNumPredictedSpectra(); energy++) {
    ...
    spec->outputToMspStream(out, id, cfg->ionization_mode, energy,
                            smiles_or_inchi, cfg->default_mz_decimal_place);
  }
}
...
void MolData::writePredictedSpectraToMgfFileStream(std::ostream &out) {
  ...
  for (unsigned int energy = 0; energy < getNumPredictedSpectra(); energy++) {
    const Spectrum *spec = getPredictedSpectrum(energy);
    spec->outputToMgfStream(out, id, cfg->ionization_mode, energy,
                            parent_ion_mw, smiles_or_inchi,
...
void MolData::outputSpectra(std::ostream &out, const char *spec_type,
  ...
  if ((std::string(spec_type) == "Predicted") && add_version) {
    ...
    if (smiles_or_inchi.substr(0, 6) == "InChI=") {
      ...
      out << "#InChiKey=" << RDKit::InchiToInchiKey(smiles_or_inchi)
          << std::endl;
    } else {
      RDKit::RWMol *rwmol;
      ...
      out << "#InChiKey="
          << RDKit::InchiToInchiKey(RDKit::MolToInchi(*rwmol, rv)) << std::endl;
      out << "#Formula=" << RDKit::Descriptors::calcMolFormula(*rwmol)
          << std::endl;
      out << "#PMass=" << std::fixed << std::setprecision(5)
          << getParentIonMass() << std::endl;
      delete rwmol;
    }
  }
...
void MolData::postprocessPredictedSpectra(double perc_thresh, int min_peaks,
                                          int max_peaks,
                                          double min_relative_intensity_prec,
                                          int quantise_peaks_decimal_place) {
  ...
  for (auto &predicted_spectrum : predicted_spectra) {
    ...
    predicted_spectrum.postProcess(perc_thresh, min_peaks, max_peaks,
                                   min_relative_intensity_prec);
...
void MolData::getSampledTransitionIdsWeightedRandomWalk(
    std::set<int> &selected_ids, int max_num_iter, int energy,
    double explore_weight) {
  if (!hasEmptySpectrum(0) && hasComputedGraph())
    fg->getSampledTransitionIdsWeightedRandomWalk(
        selected_ids, max_num_iter, thetas[energy], explore_weight);
}

void MolData::getRandomSampledTransitions(std::set<int> &selected_ids,
                                          int max_selection) {
  if (!hasEmptySpectrum(0) && hasComputedGraph())
    fg->getRandomSampledTransitions(selected_ids, max_selection);
}

void MolData::getSampledTransitionIdsRandomWalk(std::set<int> &selected_ids,
                                                int max_selection) {
  if (!hasEmptySpectrum(0) && hasComputedGraph())
    fg->getSampledTransitionIdsRandomWalk(selected_ids, max_selection);
}

void MolData::getSampledTransitionIdUsingDiffMapBFS(
    std::set<int> &selected_ids, std::set<unsigned int> &selected_weights) {
  if (!hasEmptySpectrum(0) && hasComputedGraph())
    fg->getSampledTransitionIdsDiffMapChildOnly(selected_ids, selected_weights);
}

void MolData::getSampledTransitionIdUsingDiffMapCA(
    std::set<int> &selected_ids, std::set<unsigned int> &selected_weights) {
  if (!hasEmptySpectrum(0) && hasComputedGraph())
...
void MolData::getSelectedMasses(std::set<unsigned int> &selected_weights,
                                int energry_level) {

  Comparator *cmp = new Dice(cfg->ppm_mass_tol, cfg->abs_mass_tol);
  std::vector<peak_pair_t> peak_pairs;
  cmp->getMatchingPeakPairsWithNoneMatchs(peak_pairs, &spectra[energry_level],
                                          &predicted_spectra[energry_level]);

  std::map<double, double, std::greater<double>> difference;
  for (const auto &peak_pair : peak_pairs) {
    double intensity_difference =
        std::fabs(peak_pair.first.intensity - peak_pair.second.intensity);
    ...
    if (intensity_difference > cfg->ga_diff_sampling_difference) {
      ...
      difference.insert(
          std::pair<double, double>(intensity_difference, peak_mass));
    }
  }
  delete (cmp);
  ...
  for (const auto &diff : difference) {
    ...
    selected_weights.insert(
        (unsigned int)std::round(diff.second * WEIGHT_SELECTION_SCALER));
  }
}
