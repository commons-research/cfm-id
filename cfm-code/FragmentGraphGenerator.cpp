...
FragmentGraph *FragmentGraphGenerator::createNewGraph(config_t *cfg) {
  ...
  id_depth_computed_cache
      .clear(); // The graph is empty, so clear all computation records
...
// caller to delete
FragmentTreeNode *
FragmentGraphGenerator::createStartNode(std::string &smiles_or_inchi,
                                        int ionization_mode) {
  ...
  if (!rwmol)
    throw RDKit::SmilesParseException(
        "Error occurred - assuming Smiles Parse  Exception");
  ...
  if (!fh) {
    if (!fc) {
      throw std::runtime_error("FeatureCalculator (fc) is null, unable to create FeatureHelper");
    }
  ...
  if (num_frags - num_ionic != 1) {
    std::cerr << "Unsupported input molecule: Too many starting fragments in "
  ...
  auto start_node =
      new FragmentTreeNode(romol_ptr_t(rwmol), num_ep, 0, fh, e_loc);
  return start_node;
}

int FragmentGraphGenerator::countExtraElectronPairs(
    RDKit::RWMol *rwmol, std::vector<int> &output_e_loc) {
  ...
  for (ai = rwmol->beginAtoms(); ai != rwmol->endAtoms(); ++ai) {
    ...
    output_e_loc[(*ai)->getIdx()] =
        implied_val - Hs -
        (*ai)->getDegree(); // Record where the extra electrons were
  }
...
bool FragmentGraphGenerator::alreadyComputed(int id, int remaining_depth) {
  if (id_depth_computed_cache.find(id) ==
          id_depth_computed_cache.end() // Not found
      || id_depth_computed_cache[id] <
             remaining_depth) { // Or computed previously higher on the tree
...
void FragmentGraphGenerator::compute(FragmentTreeNode &node,
                                     int remaining_depth, int parent_id,
  ...
  if (current_graph->getNumFragments() > MAX_FRAGMENTS_PER_MOLECULE ||
    ...
    std::cout << "Maximum number of fragments or transitions exceeded."
              << std::endl;
    throw FragmentGraphMaxSizeExceededException();
  }

  if (verbose)
    std::cout << current_graph->getNumFragments() << ":"
              << RDKit::MolToSmiles(*node.ion.get()) << std::endl;
  ...
  if (parent_id < 0) // Break from Precursor
    h_loss_allowed = current_graph->includesHLossesPrecursorOnly() ||
                     current_graph->includesHLosses();
  else // Break from Non-Precursor
    h_loss_allowed = !(current_graph->includesHLossesPrecursorOnly()) &&
                     current_graph->includesHLosses();
  node.generateBreaks(breaks, h_loss_allowed,
                      current_graph->allowCyclization());
  ...
  for (auto &brk : breaks) {
    ...
    for (int ifrag_idx = 0; ifrag_idx < brk.getNumIonicFragAllocations();
         ifrag_idx++) {
      ...
      std::vector<int> current_brk_child_depths(added_child_count,
                                                child_remaining_depth);
      child_remaining_depth_vector.insert(child_remaining_depth_vector.end(),
                                          current_brk_child_depths.begin(),
                                          current_brk_child_depths.end());

      std::vector<int> current_brk_child_remaining_rings(
          added_child_count, child_remaining_ring_breaks);
      child_remaining_ring_breaks_vector.insert(
          child_remaining_ring_breaks_vector.end(),
  ...
  for (int child_idx = 0; child_idx < node.children.size(); ++child_idx) {
    compute(node.children[child_idx], child_remaining_depth_vector[child_idx],
            id, child_remaining_ring_breaks_vector[child_idx]);
  }
...
// Compute a FragmentGraph starting at the given node and computing to the depth
// given. The output will be appended to the current_graph
void LikelyFragmentGraphGenerator::compute(FragmentTreeNode &node,
                                           int remaining_depth, int parentid,
  ...
  int id = current_graph->addToGraphWithThetas(node, node.getAllTmpThetas(),
                                               parentid);
  ...
  for (auto child = node.children.begin(); child != node.children.end();
    ...
    FeatureVector *fv = fc->computeFeatureVector(
        tmp_t.getIon(), tmp_t.getNeutralLoss(), node.ion);
  ...
  for (auto itt = node.children.begin(); itt != node.children.end();
       ++itt, child_idx++) {
    max_child_prob = log_prob_thresh - 10.0;
    for (int energy = 0; energy < denom.size(); energy++) {
      // normal prob + dups (last term)
      double child_log_prob =
          itt->getTmpTheta(energy) - denom[energy] + parent_log_prob;
      ;
...
void FragmentGraphGenerator::applyIonization(RDKit::RWMol *rwmol,
                                             int ionization_mode) {
  ...
  boost::tuple<bool, bool, bool> alreadyq_oktogo =
      FragmentTreeNode::findAlreadyChargedOrSplitCharge(pindx_nidx_ridx, *rwmol,
                                                        0, rad_side, is_neg);
  if (boost::get<0>(alreadyq_oktogo) && !boost::get<1>(alreadyq_oktogo)) {
    ...
    std::pair<int, int> qidx_ridx =
        FragmentTreeNode::findChargeLocation(*rwmol, 0, rad_side, is_neg);
    if (qidx_ridx.first < 0) {
      std::cerr << "Could not ionize - no location found for charge"
                << std::endl;
      throw IonizationException();
    } else if (qidx_ridx.second < 0 && rad_side >= 0) {
      std::cerr << "Could not ionize - no location found for radical"
                << std::endl;
      throw IonizationException();
    }
    try {
      FragmentTreeNode::assignChargeAndRadical(*rwmol, qidx_ridx.first,
                                               qidx_ridx.second, is_neg);
      RDKit::MolOps::sanitizeMol(
          *rwmol); // Re-sanitize...sometimes RDKit only throws the exception
                   // the second time...
...
int LikelyFragmentGraphGenerator::alreadyComputedProb(int id,
                                                      double prob_offset) {
  if (id_prob_computed_cache.find(id) == id_prob_computed_cache.end() ||
      id_prob_computed_cache[id] <
          prob_offset) { // Or computed previously with lower prob
