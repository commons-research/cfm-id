...
Param::Param(const std::vector<std::string> &a_feature_list, int a_num_energy_levels)
...
void Param::appendNextEnergyParams(Param &next_param, int energy) {
	...
	const std::vector<float> &new_weights = next_param.getWeights();
	for (unsigned int i = 0; i < num_new_weights; i++) weights[start_offset + i] = new_weights[i + new_start_offset];
}
...
void Param::appendRepeatedPrevEnergyParams() {
	...
	weights.resize(num_energy_levels * num_per_e_level);
	for (unsigned int i = 0; i < num_per_e_level; i++) weights[to_offset + i] = weights[from_offset + i];
}
...
void Param::zeroInit() {
	...
	for (unsigned int i = 1; i < weights.size(); i++) weights[i] = 0.0;
...
void Param::fullZeroInit() {
	...
	// All Terms 0.0
	for (unsigned int i = 0; i < weights.size(); i++) weights[i] = 0.0;
}
...
void Param::saveToFile(std::string &filename) {
	...
	if (!out.is_open()) {
		...
		int num_used                     = 0;
		for (; itt != weights.end(); ++itt) num_used += (*itt != 0);
		...
		for (; it != feature_list.end(); ++it) out << *it << " ";
		out << std::endl;
