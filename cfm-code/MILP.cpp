...
int MILP::runSolver(std::vector<int> &output_bmax, bool allow_lp_q,
                    int max_free_pairs, bool allow_rearrangement) {
  ...
  if (ret == 0) {
    ...
    for (i = 0; i < num_bonds && ret == 0; i++) {
      ...
      int limit =
          0; // bonds that are broken or in the other fragment are limited to 0
      int end_lp_limit = 0,
          begin_lp_limit =
              0; // bonds for which there is no lone pair to donate (or broken,
                 // or in other fragment) are limited to 0
    ...
    // Add constraints for neighbouring ring bonds (can't have two double in a
    // row)
    ...
    for (int ringidx = 0; bit != brings.end() && ret == 0; ++bit, ringidx++) {
      ...
      RDKit::Bond *bond =
          kekulized_mol.getBondWithIdx(*(bit->begin())); // Starting Bond
    ...
    for (i = 0; i < num_atoms && ret == 0; i++) {
      ...
      row[0] = 1;
      int hloss_allowed = (!has_lp) && (fragidx == fragmentidx) &&
                          (ionic_q == 0) && (num_ur == 0);
      ...
      if (j > 0) {
        ...
        if (atom->getDegree() > origval)
          val_limit = atom->getDegree();
    ...
    // Add the maximum constraint (no point finding a solution with more
    // electrons than we have)
    if (ret == 0) {
      for (int j = 0; j < Ncol; j++) {
        colno[j] = j + 1;
        row[j] = 1;
      }
      if (!add_constraintex(lp, Ncol, row, colno, LE,
                            max_free_pairs + min_single_bonds))
        ret = 3;
#ifdef __DEBUG_CONSTRAINTS__
  ...
  if (ret == 0) {
    // set the objective function = sum bond electrons + sum lone pairs used for
    // bonding
  ...
  if (ret == 0) {
    ...
    // std::cout << get_Ncolumns(lp) << " " << get_Norig_columns(lp) <<
    // std::endl;
    ret = solve(lp);
  ...
  if (ret == 0) {
    ...
    for (int j = 0; j < Ncol; j++) {
      ...
      // std::cout << "Column " << j << " has value " <<  (int) row[j] <<
      // std::endl;
    }
  ...
  // Condense the atom H loss charge position results to a single index (or -1
  // if none)
  int hloss_idx[2] = {-1, -1};
...
RDKit::Bond *MILP::getNextBondInRing(RDKit::Bond *bond, RDKit::Atom *atom,
                                     std::vector<int> &ring_bond_flags) {
