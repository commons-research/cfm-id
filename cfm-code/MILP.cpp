...
int MILP::runSolver(std::vector<int> &output_bmax, bool allow_lp_q, int max_free_pairs, bool allow_rearrangement) {
	...
	auto kekulized_mol = RDKit::RWMol(*_mol);
	...
	lp            = make_lp(0, Ncol);
	if (lp == nullptr) ret = 1; /* couldn't construct a new model... */

	if (ret == 0) {
		colno = (int *)malloc(Ncol * sizeof(*colno));
		row   = (REAL *)malloc(Ncol * sizeof(*row));
		if ((colno == nullptr) || (row == nullptr)) ret = 2;
	}
	...
	if (ret == 0) {
		...
		for (i = 0; i < num_bonds && ret == 0; i++) {
			...
			int end_lp_limit = 0, begin_lp_limit = 0; // bonds for which there is no lone pair to donate (or broken, or
			                                          // in other fragment) are limited to 0
			bond->getProp("Broken", broken);
			RDKit::Atom *begin_atom = bond->getBeginAtom();
			begin_atom->getProp("FragIdx", fragidx);
			int min_limit = 0;
			if (!broken && fragidx == _fragmentidx) {
		...
		for (int ringidx = 0; bit != brings.end() && ret == 0; ++bit, ringidx++) {
			...
			if (ringidx == _broken_ringidx) continue;
			row[0] = 1;
			row[1] = 1;
			row[2] = 1;
			row[3] = 1;
			...
			while (ret == 0 && (first_flag || prev_bond != start_bond)) {
				...
				colno[0] = static_cast<int>(prev_bond->getIdx() + 1);
				colno[1] = static_cast<int>(bond->getIdx() + 1);
				colno[2] = static_cast<int>(prev_bond->getIdx() + 1 + num_bonds);
				colno[3] = static_cast<int>(bond->getIdx() + 1 + num_bonds);
		...
		for (i = 0; i < num_atoms && ret == 0; i++) {
			...
			row[0]            = 1;
			int hloss_allowed = (!has_lp) && (fragidx == _fragmentidx) && (ionic_q == 0) && (num_ur == 0);
			if (!add_constraintex(lp, 1, row, colno, LE, hloss_allowed)) ret = 3;
#ifdef __DEBUG_CONSTRAINTS__
			printConstraint(1, colno, false, hloss_allowed);
#endif
			if (ret != 0) break;

			if (fragidx != _fragmentidx) continue;
			...
			for (; cbond_beg != cbond_end; ++cbond_beg) {
				// for (; it != ip.second; ++it) {
				RDKit::Bond *cbond = (*_mol)[*cbond_beg];
				int cbond_broken;
				cbond->getProp("Broken", cbond_broken);
				if (cbond_broken) continue;

				colno[j] = static_cast<int>(cbond->getIdx() + 1); // variable idx i.e. bond
				row[j++] = 1;                                     // multiplier
				...
				if (cbond->getBeginAtomIdx() != i) {
					colno[j] = static_cast<int>(cbond->getIdx() + 1 + num_bonds);
					row[j++] = 1;
				}
				if (cbond->getEndAtomIdx() != i) {
					colno[j] = static_cast<int>(cbond->getIdx() + 1 + 2 * num_bonds);
					row[j++] = 1;
				}
			}
			if (j > 0) {
				unsigned int val_limit = origval;
				if (atom->getDegree() > origval) val_limit = atom->getDegree();
				if (!add_constraintex(lp, j, row, colno, LE, val_limit)) ret = 3;
#ifdef __DEBUG_CONSTRAINTS__
		...
		if (ret == 0) {
			...
			if (!add_constraintex(lp, j, row, colno, LE, 1)) ret = 3;
#ifdef __DEBUG_CONSTRAINTS__
		...
		if (ret == 0) {
			...
			if (!add_constraintex(lp, Ncol, row, colno, LE, max_free_pairs + min_single_bonds)) ret = 3;
#ifdef __DEBUG_CONSTRAINTS__
	...
	if (ret == 0) {
		for (int col_idx = 0; col_idx < Ncol; col_idx++) { set_int(lp, col_idx + 1, TRUE); }
	}
	...
	if (ret == 0) {
		...
		int obj = static_cast<int>(get_objective(lp));
		// std::cout << "obj value " <<  obj << std::endl;

		output_max_e = obj - min_single_bonds;
	} else {
		for (int j = 0; j < Ncol; j++) output_bmax[j] = 0;
	}
	...
	for (int j = 0; j < num_atoms; j++) {
		if (output_bmax[j + 3 * num_bonds]) hloss_idx[_fragmentidx] = j;
	}
...
RDKit::Bond *MILP::getNextBondInRing(RDKit::Bond *bond, RDKit::Atom *atom, std::vector<int> &ring_bond_flags) {
	...
	for (; beg_abond != end_abond; ++beg_abond) {
		int idx = static_cast<int>((*_mol)[*beg_abond]->getIdx());
		if (ring_bond_flags[idx] && idx != bond->getIdx()) return atom->getOwningMol().getBondWithIdx(idx);
	}
	return nullptr; // Error@!
}

void MILP::printConstraint(int num_terms, int *colno, bool ge, int val) const {
	if (!_verbose) return;
