...
FeatureCalculator::FeatureCalculator(std::string &config_filename) {
	...
	if (!ifs.good()) { std::cout << "Trouble opening feature config file: " << config_filename << std::endl; }
	while (ifs.good()) {
...
FeatureCalculator::FeatureCalculator(std::vector<std::string> &feature_list) {
	...
	for (auto feature_name : feature_list) configureFeature(feature_name);
...
const std::vector<std::string> FeatureCalculator::getValidFeatureNames() {
	...
	for (const auto &break_feature : breakFeatureCogs()) output.push_back(break_feature.getName());

	for (const auto &frag_feature : fragmentFeatureCogs()) output.push_back(frag_feature.getName());
...
unsigned int FeatureCalculator::getNumFeatures() {
	...
	for (; it != used_break_feature_idxs.end(); ++it) {
		...
		if (breakFeatureCogs()[*it].getName() == "QuadraticFeatures") quadratic = 1;
	}
...
FeatureVector *FeatureCalculator::computeFeatureVector(const RootedROMol *ion, const RootedROMol *nl,
	...
	for (const auto &feature_idx : used_break_feature_idxs) {
		...
		try {
			...
			std::cout << "Could not compute " << feature->getName() << std::endl;
			throw FeatureCalculationException("Could not compute " + feature->getName());
		}
	}
