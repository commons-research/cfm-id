...
double EmModel::trainModel(std::vector<MolData> &molDataSet, int group, std::string &out_param_filename,
	...
	if (!initial_params_provided) param->initWeights(cfg->param_init_type);
...
#pragma omp parallel for reduction(max : max_pob_dice, max_pob_dp, max_pob_precision, max_pob_recall)                  \
    reduction(+ : num_training_mols, num_val_mols) num_threads(cfg->num_threads)
	for (auto &mol_data : molDataSet) {
		...
		if (mol_data.getGroup() != validation_group) {
			auto [dice_internal, dp_internal, precision_internal, recall_internal] =
			    computeMetrics(mol_data.getOrigSpectrum(energy_level), mol_data.getSpectrum(energy_level), max_pob_dice,
			                   max_pob_dp, max_pob_precision, max_pob_recall);
			max_pob_dice      = dice_internal;
			max_pob_dp        = dp_internal;
			max_pob_precision = precision_internal;
			max_pob_recall    = recall_internal;
...
#pragma omp parallel for num_threads(cfg->num_threads)
		for (int molidx = 0; mol_it != molDataSet.end(); ++mol_it, molidx++) {
			...
			if (!mol_it->hasComputedGraph()) continue; // If we couldn't compute it's graph for some reason..
			...
			if (mol_it->getGroup() == validation_group && cfg->disable_cross_val_metrics) continue;
		...
		if (comm->isMaster()) std::cout << "[M-Step]Staring Learning Rate=" << learning_rate << std::endl;
		...
		//     comm->printToMasterOnly(("used cpu time:" +
		//     std::to_string(cpu_time)).c_str());
		...
		for (mol_it = molDataSet.begin(); mol_it != molDataSet.end(); ++mol_it, molidx++) {
			...
			if (mol_it->getGroup() == validation_group && !cfg->disable_cross_val_metrics) {
				...
				computeMetrics(mol_it->getOrigSpectrum(energy_level), mol_it->getPredictedSpectrum(energy_level),
				               val_dice, val_dp, val_precision, val_recall);
			} else if (mol_it->getGroup() != validation_group && !cfg->disable_training_metrics) {
				computeMetrics(mol_it->getOrigSpectrum(energy_level), mol_it->getPredictedSpectrum(energy_level),
				               train_dice, train_dp, train_precision, train_recall);
				computeMetrics(mol_it->getSpectrum(energy_level), mol_it->getPredictedSpectrum(energy_level),
				               pruned_train_dice, pruned_train_dp, pruned_train_precision, pruned_train_recall);
			}
		}
		...
		std::string q_time_msg = "[M-Step][T+" + getTimeDifferenceStr(start_time, after) +
		                         ...
		                         " seconds";
		if (comm->isMaster()) writeStatus(q_time_msg.c_str());
		...
		// check if EM meet halt flag and update lr, sampling methods and no
		// progress count
		if (comm->isMaster())
	...
	if (iter >= cfg->em_max_iterations)
		comm->printToMasterOnly(
		    ("Warning: EM still converging after " + std::to_string(iter) + " iterations.").c_str());
...
double EmModel::computeAndSyncLoss(std::vector<MolData> &data, suft_counts_t &suft, unsigned int energy) {
	...
	if (comm->isMaster() && cfg->lambda > 0.0) loss += getRegularizationTerm(energy);
...
std::tuple<double, double, double, double> EmModel::computeMetrics(const Spectrum *p, const Spectrum *q, double dice,
                                                                   double dp, double precision, double recall) {
	...
	return std::tuple<double, double, double, double>(dice, dp, precision, recall);
}
...
double EmModel::updateParametersGradientAscent(std::vector<MolData> &data, suft_counts_t &suft, float learning_rate,
	...
	if (comm->isMaster()) { solver = getSolver(cfg->ga_method, learning_rate); }
	...
	if (comm->used_idxs.empty()) {
		...
		if (cfg->collected_all_used_idx) {
			...
			for (int i = 0; i < param->getNumWeightsPerEnergyLevel(); ++i) comm->used_idxs.insert(grad_offset + i);
		}

		for (int molidx = 0; itdata != data.end(); ++itdata, molidx++) {
			if (itdata->getGroup() != validation_group) {
				if (!cfg->collected_all_used_idx) collectUsedIdx(*itdata, comm->used_idxs, energy);
	...
	while (iter++ < max_iteration && ga_no_progress_count <= cfg->ga_no_progress_count) {
		...
		for (auto batch_idx = 0; batch_idx < num_batch; ++batch_idx) {
			...
			for (auto &mol_it : data) {
				if (minibatch_flags[molidx] == batch_idx && mol_it.getGroup() != validation_group) {
					...
					num_trans += computeAndAccumulateGradient(&grads[0], molidx, mol_it, suft, sampling_method, energy);
				}
				molidx++;
			}
		...
		if (comm->isMaster()) {
			...
			if (!cfg->disable_cpu_usage_metrics) { std::cout << cpu_usage_string; }
			...
			// we are doing ga, save best weight some where and check if we have
			// getting better
