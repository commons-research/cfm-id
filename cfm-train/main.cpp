...
void trainSingleEnergyCFM(std::string &param_filename, config_t &cfg, FeatureCalculator &fc,
                          std::string &status_filename, int group, std::vector<MolData> &data, int start_energy,
                          int no_train, int start_repeat);
...
int main(int argc, char *argv[]) {
	...
	std::string
	    peakfile_dir_or_msp; // MSP file or Directory containing the peak files for each molecule (in format <id>.txt)
	...
	general_options.add_options()("help,h", "Help message")(
	    ...
	    "tmp_data/fv_fragment_graphs_folder")("no_train", po::value<bool>(&no_train)->default_value(false),
	                                          "no training flag, defualt false");
	...
	// set group
	if (min_group != -1) max_group = min_group + 1;
	...
	if (fv_fragment_graphs_folder.empty()) fv_fragment_graphs_folder = data_folder + "/fv_fragment_graphs";

	// Create the tmp_data directory if it doesn't exist
	if (!boost::filesystem::exists(data_folder)) boost::filesystem::create_directory(data_folder);
	...
	if (boost::filesystem::exists(status_filename)) boost::filesystem::remove_all(status_filename);
	...
	config_t cfg;
	initConfig(cfg, config_filename, argv[0], true);
...
#pragma omp critical
	...
	std::cout << success_count << " successfully computed. " << except_count << " exceptions." << std::endl;
	...
	if (peakfile_dir_or_msp.substr(peakfile_dir_or_msp.size() - 4, 4) == ".msp") {
		...
		if (pre_id.substr(pre_id.size() - 3, 3) == "<P>") {
			peakfile_dir_or_msp.replace(peakfile_dir_or_msp.size() - 7, 3, boost::lexical_cast<std::string>(0));
		}
	}
	...
	for (int group = min_group; group < max_group; group++) {
		std::cout << "Running EM to train parameters for Group " << group << std::endl;
		...
		if (spectra_in_msp) {
			std::string proc_group_str    = "G" + boost::lexical_cast<std::string>(group);
		...
		std::cout << "Generating Peak Predictions for Group " << group << " from " << param_filename << std::endl;
...
void trainSingleEnergyCFM(std::string &param_filename, config_t &cfg, FeatureCalculator &fc,
	...
	for (int energy = 0; energy < cfg.spectrum_depths.size(); energy++) {
		...
		if (energy >= start_energy && !no_train) {
			...
			double prev_Q = -std::numeric_limits<double>::max();
			...
			for (int repeat = start_repeat; repeat < se_cfg.num_em_restarts; repeat++) {
				...
				if (!boost::filesystem::exists(repeat_filename))
					repeat_filename = "";
	...
	final_params->saveToFile(param_filename);
	delete final_params;
}
